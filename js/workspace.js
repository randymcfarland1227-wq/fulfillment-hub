...
const progressText=t=>`${t.answered} answered · ${t.partial} in progress · ${t.unanswered} not started${t.drafted?` · ${t.drafted} drafts`:''}`;
...
function focusView(p){
 ...
 <aside class="reflection-aside"><h2>Your words, in context</h2><p>Revisit a memory or example without leaving this question.</p>${evRow(p.evidence,'Source passages')||'<p class="caption">No source passage is mapped to this question yet.</p>'}<details><summary>What does this status mean?</summary><p>Answered: the main question is covered. In progress: a real answer is already here and only needs refining. Not started: no substantive answer yet — each one says what it is waiting for. Draft saved: you have written here but have not marked it answered.</p></details><a class="text-link" href="#/writing">Back up your writing →</a></aside></div></section>`;
}
